                                ...
                                @if (!empty($selectedServices))
                                    ...
                                    @foreach ($selectedServices as $service)
                                        <p>{{ $service->name }}
                                            ...
                                            x {{ $serviceQuantities[$service->id] ?? 1 }}</p>
                                    @endforeach
                                @endif
                            </div>
                            <div class="lh-check-block-content mb-3">
                                ...
                                <div class="d-flex justify-content-between">
                                    ...
                                    <p id="base-price-display">
                                        {{ \App\Helpers\FormatHelper::formatPrice($basePrice + $serviceTotal) }}</p>
                                </div>
                                <div class="d-flex justify-content-between">
                                    <p>Thuế và phí (8%)</p>
                                    <p id="initial-tax-display">
                                        {{ \App\Helpers\FormatHelper::formatPrice(($basePrice + $serviceTotal) * 0.08) }}
                                    </p>
                                </div>
                                ...
                                <div id="discount-section" style="display: {{ $discountAmount > 0 ? 'block' : 'none' }};">
                                    ...
                                    <div class="d-flex justify-content-between">
                                        ...
                                        <p id="discount-amount">
                                            {{ $discountAmount > 0 ? '- ' . \App\Helpers\FormatHelper::formatPrice($discountAmount) : '' }}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div id="after-discount-section"
                                    style="display: {{ $discountAmount > 0 ? 'block' : 'none' }};">
                                    <div class="d-flex justify-content-between">
                                        <p>Giá phòng & dịch vụ (sau áp mã)</p>
                                        <p id="after-base-price-display">
                                            {{ \App\Helpers\FormatHelper::formatPrice($basePrice + $serviceTotal - $discountAmount) }}
                                        </p>
                                    </div>
                                    <div class="d-flex justify-content-between">
                                        <p>Thuế và phí (8%)</p>
                                ...
                                <div class="d-flex justify-content-between mt-2">
                                    <input type="text" id="promotion-code" class="form-control w-50"
                                        placeholder="Nhập mã giảm giá">
                                    <div>
                                        <button type="button" id="apply-promotion-btn" class="btn btn-outline-primary">Áp
                                            dụng</button>
                                        <button type="button" id="cancel-promotion-btn" class="btn btn-outline-danger"
                                ...
                                <input type="hidden" id="sub_total_after_default"
                                    value="{{ $basePrice + $serviceTotal - $discountAmount }}">
                            </div>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="lh-checkout-inner">
                                <div class="lh-checkout-wrap mb-24">
                                    ...
                                    <div class="lh-check-block-content">
                                        <form id="confirm-form" method="POST" action="{{ route('bookings.store') }}"
                                            ...
                                            <div id="method-payment-section">
                                                ...
                                                <div class="form-check">
                                                    <input class="form-check-input payment-method" type="radio"
                                                        name="payment_method" id="payment2" value="online" checked>
                                                    <label class="form-check-label" for="payment2">Thanh toán trực
                                                        tuyến</label>
                                                </div>
                                            </div>
                                            ...
                                            <div id="online-payment-section" style="display: block; margin-left: 20px;">
                                                <div class="form-check">
                                                    <input class="form-check-input online-payment-method" type="radio"
                                                        name="online_payment_method" id="vnpay" value="vnpay"
                                                        checked>
                                                    <label class="form-check-label" for="vnpay">
                                                        <img src="https://vnpay.vn/s1/statics.vnpay.vn/2023/6/0oxhzjmxbksr1686814746087.png"
                                                            alt="VNPay" class="payment-icon"> VNPay Thanh toán qua
                                                        VNPay
                                                    </label>
                                                </div>
                                            </div>
    ...
    <script>
        $(document).ready(function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#voucher-amount').text(voucherDiscount.toLocaleString('vi-VN') +
                                '- VND');
                            $('#voucher-section').show();

                            $('#after-base-price-display').text(subTotalAfterVoucher
                                .toLocaleString('vi-VN') + ' VND');
                            $('#tax-fee-display').text(taxAfterVoucher.toLocaleString('vi-VN') +
                                ' VND');
                            $('#after-discount-section').show();

                            $('#total_price_display').text(totalAfterVoucher.toLocaleString(
                                'vi-VN') + ' VND');
                            ...
                            $('#promotion-message').html('<p class="text-success">' + response
                                .message + '</p>');
                        } else {
                            $('#promotion_id').val('');
                            $('#confirm-button').prop('disabled', true);
                            $('#voucher-section').hide();
                            $('#promotion-message').html('<p class="text-danger">' + response
                                .message + '</p>');
                        }
                    },
                    error: function() {
            ...
            $('#cancel-promotion-btn').on('click', function() {
                ...
                if (defaultDiscount > 0) {
                    $('#after-base-price-display').text(subTotalAfterDefault.toLocaleString('vi-VN') +
                        ' VND');
            ...
            $('.online-payment-method').on('change', function() {
                ...
                if (onlineMethod === 'vnpay') {
                    $('#payment-instruction p').text(
                        'Vui lòng lưu ý hiện thanh toán qua cổng thanh toán: VNPay');
                }
                $('#momo-qr-section').hide();
            });
