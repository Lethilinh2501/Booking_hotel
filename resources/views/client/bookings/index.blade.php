                    ...
                    <div class="room-list">
                        @forelse ($bookings as $booking)
                            <a href="{{ route('bookings.show', $booking->id) }}"
                                class="room-item-link text-decoration-none">
                                <div class="room-item card shadow-sm mb-4 position-relative"
                                    ...
                                    <div class="card-body d-flex align-items-center p-3">
                                        <div class="room-image" style="flex: 0 0 130px;">
                                            @if (
                                                $booking->rooms->isNotEmpty() &&
                                                    $booking->rooms->first()->roomType &&
                                                    $booking->rooms->first()->roomType->roomTypeImages->isNotEmpty())
                                                @php
                                                    $mainImage = $booking->rooms
                                                        ->first()
                                                        ->roomType->roomTypeImages->where('is_main', true)
                                                        ->first();
                                                    $imageUrl = $mainImage
                                                        ? Storage::url($mainImage->image)
                                                        : 'https://cf.bstatic.com/xdata/images/hotel/max1024x768/249146036.jpg';
                                                @endphp
                                                <img src="{{ $imageUrl }}" alt="Room Image"
                                                    style="width: 100%; height: 100px; object-fit: cover; border-radius: 8px 0 0 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
                                            @else
                                                <img src="https://cf.bstatic.com/xdata/images/hotel/max1024x768/249146036.jpg"
                                                    alt="Room Image"
                                                    style="width: 100%; height: 100px; object-fit: cover; border-radius: 8px 0 0 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
                                            @endif
                                        </div>
                                        <div class="room-details flex-grow-1 px-4">
                                            <h5 class="mb-1 text-dark">
                                                {{ $booking->rooms->first()->roomType->name ?? 'Chưa có loại phòng' }}</h5>
                                            <p class="mb-1 text-muted">
                                                {{ \App\Helpers\FormatHelper::formatDateVI($booking->check_in) }} –
                                                {{ \App\Helpers\FormatHelper::formatDateVI($booking->check_out) }}</p>
                                            <span
                                                class="status-badge {{ \App\Helpers\BookingStatusHelper::getStatusClass($booking->status) }}">
                                                {{ \App\Helpers\BookingStatusHelper::getStatusLabel($booking->status) }}
                                            </span>
                                        </div>
                                        <div class="room-price d-flex align-items-center pe-3 position-relative"
                                            style="min-width: 180px; color: #555;">
                                            <div class="d-flex align-items-center">
                                                <h4 class="mb-5 text-dark" style="font-size: 22px;">VND
                                                    {{ number_format($booking->total_price, 0, ',', '.') }}</h4>
                                                <span class="vertical-dots ms-4 mb-5"
                                                    style="color: #555; font-size: 30px; font-weight: 800; cursor: pointer; transition: color 0.3s ease;"
                                                    data-booking-id="{{ $booking->id }}">⋮</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.addEventListener('click', function(e) {
                dropdowns.forEach(dropdown => {
                    if (!dropdown.contains(e.target) && !e.target.classList.contains(
                            'vertical-dots')) {
